...
contract SNXRewardUniLPStrategy is StrategyBase {
  ...
  StrategyBase(_storage, _underlying, _vault, _rewardToken, _quickRouter)
  public {
  ...
  */
  ...
  function _liquidateReward() internal {
    ...
    if (remainingRewardBalance > 0 // we have tokens to swap
      ...
      IERC20(rewardToken).safeApprove(quickRouter, 0);
      IERC20(rewardToken).safeApprove(quickRouter, remainingRewardBalance);
      ...
      IERC20(uniLPComponentToken0).safeApprove(quickRouter, 0);
      IERC20(uniLPComponentToken0).safeApprove(quickRouter, token0Amount);

      IERC20(uniLPComponentToken1).safeApprove(quickRouter, 0);
      IERC20(uniLPComponentToken1).safeApprove(quickRouter, token1Amount);

      uint256 liquidity;
      (,,liquidity) = IUniswapV2Router02(quickRouter).addLiquidity(
